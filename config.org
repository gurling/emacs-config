#+TITLE:Izzy's GNU Emacs Config
#+AUTHOR: Izzy
#+DESCRIPTION: Izzy's personal Emacs config
...
#+BEGIN_SRC emacs-lisp
  ...
  (with-eval-after-load 'evil
    ...
    (general-create-definer izzy/leader-keys
    :states '(normal insert visual motion emacs)
    :keymaps 'override
    :prefix "SPC" ;; set leader
    :global-prefix "M-SPC") ;; access leader in insert mode

    (general-create-definer izzy/keys
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; ;; Needed for the "recents" section.
  ;;(recentf-mode 1)
#+END_SRC
...
;; Hyprland / Hyprlang (Tree-sitter mode when available)
(use-package hyprlang-ts-mode
  :straight t
  :if (and (fboundp 'treesit-available-p) (treesit-available-p))
  :mode (("\\.hl\\'" . hyprlang-ts-mode)
         ("/hypr/.*\\.conf\\'" . hyprlang-ts-mode)
         ("hypr\\(?:land\\|lock\\|idle\\)\\.conf\\'" . hyprlang-ts-mode))
  :custom
...
;; Fallback: if tree-sitter is unavailable, at least use conf-mode for Hypr configs.
(unless (and (fboundp 'treesit-available-p) (treesit-available-p))
  (dolist (pattern '("/hypr/.*\\.conf\\'"
                     "hypr\\(?:land\\|lock\\|idle\\)\\.conf\\'"))
